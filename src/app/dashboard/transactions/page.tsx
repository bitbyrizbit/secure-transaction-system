...
interface Transaction {
  ...
  amount: string; // Decimal serialises to string over JSON
  status: string;
}
...
export default function TransactionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      const tx = res.transaction as unknown as Transaction;
      ...
      <div className="bg-white p-6 rounded-lg shadow border mb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
          ...
          <div>
            ...
            <select value={type} onChange={e => setType(e.target.value as TransactionType)} className="w-full border p-2 rounded">
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y">
            {loading ? (
              ...
              transactions.map((tx) => (
                <tr key={tx.id}>
                  <td className="p-4 text-sm text-slate-600 font-mono">{tx.id}</td>
